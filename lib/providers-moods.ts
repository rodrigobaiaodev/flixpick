/** Mood slug → TMDB genre IDs (comma-AND on discover). */
export const MOOD_TO_GENRE_IDS: Record<string, number[]> = {
  "action-packed": [28, 12],
  "need-to-laugh": [35],
...
export function getMoodGenreIds(moodSlug: string): number[] {
  if (isWorldCinemaMood(moodSlug)) return [];
  return MOOD_TO_GENRE_IDS[moodSlug] ?? [];
}

export function isValidMoodSlug(moodSlug: string): boolean {
  return isWorldCinemaMood(moodSlug) || moodSlug in MOOD_TO_GENRE_IDS;
}
